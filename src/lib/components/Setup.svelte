<script>
	...
	async function requestSessionKey() {
		...
		try {
			const { access_key } = await client.requestNewPermissions("miia-web", required_permissions);
	...
	async function validateKey() {
		...
		error_ref = "";
		try {
			const { human_description } = await client.verifyAccessKey(selected_key);
			localStorage.setItem("access_key", JSON.stringify(selected_key));
...
<section class="container">
	...
	>
		...
		<div class="input-group">
			{#if !entered_key}
				<div class="request-new-key">
					<button class="btn btn-primary" on:click={requestSessionKey}> Request Key </button>
				</div>
			{/if}
			<input
				id="key-input"
		...
		<div id="key-help" class="form-text">The generated API key from your Hydrus Client</div>
	</form>
	{#if !address_validated}
		{#if connecting}
			...
			<div class="alert alert-danger alert-dismissible fade show" role="alert">
				...
				<div class="row">
					<div class="col col-12 col-md-7 col-lg-8">
						<h4>Fixes</h4>
						<p>
							Depending on what the Test Connection box says you can fix the problem in various
							ways.
						</p>
						<div class="accordian">
							<div class="card">
								<details>
									...
									<div class="card-body">
										<p>
											To authorize your connection, you'll need to tell your browser to trust your
											Hydrus Client's self-signed HTTPS certificate.
										</p>
										<p>
											In the Test Connection box, you'll need to allow the connection. This varies
											by browser but you'll press something like:
										</p>
										<ol>
											<li class="m-2">
												<span class="btn btn-secondary btn-sm disabled"> Advanced... </span>
											</li>
											<li class="m-2">
												...
												<ul>
													<li>
														<p>
															...
															<a href={address} target="_blank" class="btn btn-primary">
																click here
															</a>
															and try again.
														</p>
							...
							<div class="card">
								<details>
									...
									<div class="card-body">
										<p>Make sure that you're allowing Hydrus Client through your OS's firewall.</p>
										<p>
											...
											>
												University of Central Arkansas has a decent guide on finding your local IP
												address.
											</a>
										...
										<ul>
											<li>
												...
												<span> Hydrus Client is directly on the same computer </span>
											</li>
											<li>
											...
											<li>
												...
												<ul>
													<li>
														<span>
															Hydrus Client is on your LAN and you are accessing from an iOS or
															MacOS device
														</span>
													</li>
												</ul>
											...
											<li>
												...
												<ul>
													<li>
														<span>
															Hydrus Client is on your LAN and you are accessing from a Windows
															device that is set to "Private Network"
														</span>
													</li>
												</ul>
											</li>
							...
							<div class="card">
								<details>
									...
									<div class="card-body">
										<p>
											Your address and port are correct, but you're connecting to Hydrus over HTTP
											instead of HTTPS.
										</p>
										<p>
											Make sure that
	...
	{#if key_request_failed || key_validate_failed}
		<div class="alert alert-danger" role="alert">
			<h4 class="alert-heading">
				Error {key_request_failed ? "while requesting a new key" : ""}
				{key_validate_failed ? "while validating your key" : ""}
			</h4>
			...
			{#if key_validate_failed}
				...
				<p>
					...
					<button class="btn btn-primary btn-sm" on:click={requestSessionKey}> Request Key </button>
				</p>
			{/if}
		</div>
	{/if}
