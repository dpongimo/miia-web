<script>
	...
	function isFileReadyToLoad(target_index) {
		return target_index >= index - loaded_range && target_index <= index + loaded_range;
	}
	...
	<div class="background">
        ...
        {/if} -->
			{#each file_ids as this_id, this_i (this_id)}
				<div class="file" class:current={index === this_i} bind:this={dom_file_refs[this_i]}>
					{#await getMetadata(this_id)}
						<div class="spinner-border" role="status" title="Querying {this_id}'s metadata" />
					{:then metadata}
						<!-- <p style="color: yellow">
              From
              <code>file_ids[{file_ids.length}]</code>
